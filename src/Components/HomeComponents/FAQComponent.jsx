      ...
      <div className="max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-start">
          ...
          <div className="lg:col-span-5 lg:sticky top-[120px]">
            <div className="outfit text-[var(--orange)] text-sm tracking-[0.2em] uppercase font-semibold mb-6" data-aos="fade-right">
               Information
            </div>
            <h1 data-aos="fade-right" className="outfit text-4xl sm:text-5xl md:text-7xl text-[var(--green)] leading-[1.1] tracking-tight mb-8">
              Frequently <br className="hidden md:block"/><span className="italic text-[var(--orange)]">Asked Questions</span>
            </h1>
            <p data-aos="fade-right" className="outfit text-gray-700 text-lg font-light leading-relaxed mb-12">
               Everything you need to know about our global export logistics, product purity, and supply chain operations.
            </p>
            <div data-aos="fade-right" className="hidden md:block relative w-full h-[500px] p-2">
          ...
          <div className="lg:col-span-7 w-full pt-4">
            <div className="border-t border-[var(--green)]/20">
              {faqs.map((faq, index) => (
                <div
                  data-aos="fade-left"
                  data-aos-delay={index * 50}
                  key={index}
                  className="border-b border-[var(--green)]/20 overflow-hidden group"
                >
                  <button
                    onClick={() => toggleFaq(index)}
                    className="w-full cursor-pointer py-8 flex justify-between items-center text-left"
                  >
                    <span className={`outfit text-xl sm:text-2xl md:text-3xl transition-colors duration-500 pr-4 sm:pr-8 ${openFaq === index ? 'text-[var(--orange)]' : 'text-[var(--green)] group-hover:text-[var(--orange)]'}`}>
                      {faq.question}
                    </span>
                    <span className="flex-shrink-0 text-[var(--green)]">
                      {openFaq === index ? (
                        <Minus strokeWidth={1} className="w-8 h-8" />
                      ) : (
                        <Plus strokeWidth={1} className="w-8 h-8 text-[var(--green)]/50 group-hover:text-[var(--green)] transition-colors" />
                      )}
                    </span>
                  </button>
                  ...
                  >
                    <div className="pb-10 pt-2 lg:pr-16">
                      <p className="outfit text-gray-600 leading-relaxed font-light text-base sm:text-lg">
                        {faq.answer}
                      </p>
                    </div>
                  </div>
